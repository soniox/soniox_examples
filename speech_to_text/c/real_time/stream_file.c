...
typedef struct {
    ...
    char final_text[1 * 1024 * 1024];
} client_data_t;

void *read_audio_thread(void *arg) {
    client_data_t *data = (client_data_t *)arg;

    FILE *audio_file = fopen(data->file_to_transcribe, "rb");
    if (!audio_file) {
    ...
    switch (reason) {
        ...
        case LWS_CALLBACK_CLIENT_WRITEABLE:
            ...
            if (data->write_start) {
                ...
                json_object_object_add(start_req, "api_key", json_object_new_string(data->api_key));
                ...
                json_object_object_add(start_req, "language_hints", language_hints_array);
                ...
                for (size_t i = 0; i < json_object_array_length(tokens_array); i++) {
                    ...
                    if (json_object_object_get_ex(token_obj, "text", &text_obj)) {
                        ...
                        if (text && strlen(text) > 0) {
                            struct json_object *is_final_obj;
                            if (json_object_object_get_ex(token_obj, "is_final", &is_final_obj) &&
                                json_object_get_boolean(is_final_obj)) {
                                strcat(data->final_text, text);
                            } else {
                                strcat(non_final_text, text);
                            }
                        }
                    }
                }
...
int main() {
    ...
    char *api_key = getenv("SONIOX_API_KEY");
    if (!api_key) {
        fprintf(stderr, "Environment variable SONIOX_API_KEY is not set\n");
        return 1;
    }
    char *websocket_address = "stt-rt.soniox.com";
    char *websocket_host = "stt-rt.soniox.com";
    int websocket_port = 443;
    char *websocket_path = "/transcribe-websocket";
    char *file_to_transcribe = "coffee_shop.pcm_s16le";
    ...
    data.file_to_transcribe = file_to_transcribe;
    ...
    info.protocols = (const struct lws_protocols[]){
        {"ws", callback, sizeof(client_data_t), 0},
        {NULL, NULL, 0, 0}};
    ...
    connect_info.address = websocket_address;
    connect_info.host = websocket_host;
